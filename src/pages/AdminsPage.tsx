...
const getRoleLabel = (role?: string) => {
  if (!role) return "-";
  return role.toLowerCase() === "super_admin" ? "مدير عام" : "مدير";
};
...
export default function AdminsPage() {
  ...
  const handleFormSubmit = async () => {
    ...
    if (response.success) {
      toast.success(editingAdmin ? "تم تحديث بيانات المدير بنجاح" : "تم إنشاء حساب المدير بنجاح");
      setModalOpen(false);
      ...
      toast.error(response.message || `فشل ${editingAdmin ? 'تحديث' : 'إنشاء'} المدير`);
    }
    setIsSubmitting(false);
  };
  ...
  const handleDeleteAdmin = async () => {
    ...
    if (res.success) {
      ...
      toast.error(res.message || "فشل حذف المدير");
    }
  ...
  const handleViewAdmin = async (id: number) => {
    ...
    try {
      ...
      if (res.ok && json.status) {
        ...
        toast.error("حدث خطأ أثناء جلب بيانات المدير");
      }
    } catch (err) {
  ...
  const handlePermissionCheck = (action: () => void) => {
    if (!isSuperAdmin) {
      toast.error("ليس لديك صلاحية للقيام بهذا الإجراء. هذه الصلاحية مخصصة للمدير العام فقط.");
      return;
    }
    action();
  };
    ...
    <>
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-foreground font-medium text-sm">{getRoleLabel(profile?.role)}</p>
                </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-card border rounded-xl overflow-hidden shadow-sm h-full">
            <div className="p-5 border-b flex items-center justify-between bg-muted/10">
              <h3 className="font-bold text-lg text-gray-800">المدراء المسجلين</h3>
              <Badge variant="outline">{admins.length} مدير</Badge>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40">
                    <th className="text-start px-5 py-3 font-medium text-xs text-muted-foreground uppercase tracking-wider">الاسم</th>
                    <th className="text-start px-5 py-3 font-medium text-xs text-muted-foreground uppercase tracking-wider">البريد الإلكتروني</th>
                    <th className="text-start px-5 py-3 font-medium text-xs text-muted-foreground uppercase tracking-wider">الصلاحية</th>
                    <th className="text-end px-5 py-3 font-medium text-xs text-muted-foreground uppercase tracking-wider">الإجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {loadingAdmins ? (
                    ...
                    <tr>
                      <td colSpan={4}>
                        <EmptyState message="لا يوجد مدراء آخرين مسجلين" />
                      </td>
                    </tr>
                  ) : (
                    admins.map((admin) => (
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => handleViewAdmin(admin.id)}>
                                <Eye className="h-4 w-4 ltr:mr-2 rtl:ml-2" /><span>عرض</span>
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handlePermissionCheck(() => openEditModal(admin))}>
                                <Pencil className="h-4 w-4 ltr:mr-2 rtl:ml-2" /><span>تعديل</span>
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => handlePermissionCheck(() => setDeleteAdminId(admin.id))} className="text-destructive focus:text-destructive">
                                <Trash2 className="h-4 w-4 ltr:mr-2 rtl:ml-2" /><span>حذف</span>
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </td>
      ...
      {isSuperAdmin && (
        <FormModal
          ...
          title={editingAdmin ? "تعديل بيانات المدير" : "إنشاء مدير جديد"}
          ...
          <div className="space-y-4 text-right">
            ...
            <div className="space-y-1.5">
              ...
              {editingAdmin && <p className="text-xs text-muted-foreground mt-1">اتركه فارغاً لعدم تغيير كلمة المرور.</p>}
            </div>
            <div className="space-y-1.5">
      ...
      <ConfirmDialog
        ...
        title="حذف المدير"
        description={deleteAdminId === profile?.id ? "تحذير: أنت على وشك حذف حسابك الشخصي! سيتم تسجيل خروجك فوراً ولن تتمكن من الدخول مرة أخرى. هل أنت متأكد؟" : "هل أنت متأكد من رغبتك في حذف هذا المدير؟ لا يمكن التراجع عن هذا الإجراء."}
      ...
      <Dialog open={viewModalOpen} onOpenChange={setViewModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>تفاصيل المدير</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            {loadingSingle ? (
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 p-5 rounded-xl border space-y-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-gray-200 pb-3 gap-1">
                    <span className="text-sm text-muted-foreground flex items-center gap-2 font-medium"><Mail className="h-4 w-4"/> البريد الإلكتروني</span>
                    <span className="font-semibold text-sm text-gray-800" dir="ltr">{viewAdmin.email}</span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-gray-200 pb-3 gap-1">
                    <span className="text-sm text-muted-foreground flex items-center gap-2 font-medium"><Shield className="h-4 w-4"/> الصلاحية</span>
                    <span className="font-semibold text-sm text-gray-800">{getRoleLabel(viewAdmin.role)}</span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                    <span className="text-sm text-muted-foreground flex items-center gap-2 font-medium"><Calendar className="h-4 w-4"/> تاريخ الانضمام</span>
              ...
              <div className="text-center text-muted-foreground py-8">
                لم يتم العثور على بيانات المدير
              </div>
            )}
          </div>
        </DialogContent>
